...
class OllamaService(SummarizationServiceBase):
    ...
    async def _generate_with_ollama(
        ...
        for attempt in range(max_retries):
            try:
                ...
                service_logger.warning(f"Ollama request timeout (attempt {attempt + 1})")
                if attempt == max_retries - 1:
                    raise
                await asyncio.sleep(2 ** attempt)  # Exponential backoff
                
            except Exception as e:
                service_logger.error(f"Ollama request failed (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
                    raise
    ...
    def _extract_summary_from_text(self, text: str) -> str:
        ...
        return ' '.join(summary_lines) if summary_lines else "Meeting summary not available."
    ...
    async def summarize_transcript(
        ...
        try:
            ...
            summary = MeetingSummary(
                meeting_id=meeting_id,
                summary=extracted_data.get("executive_summary", "Meeting transcript processed successfully."),
